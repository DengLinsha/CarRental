    ...
    >
      ...
      >
          <el-form-item label="汽车图片">
            <img class="img" :src="ruleForm.picUrl" alt="" style="width: 50%;" />
          </el-form-item>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item class="input" label="座位数" prop="gears">
              <el-input
                v-model="ruleForm.carSeat"
                placeholder="座位数"
                readonly
                clearable
              ></el-input>
          ...
          <el-col :span="12">
            <el-form-item class="input" label="地址" prop="gears">
              <el-input
...
export default {
  ...
  mounted() {
    // 监听事件总线
    this.$bus.$on("show-use-car-dialog", (car) => {
      this.car = car;
  ...
  beforeDestroy() {
    // 移除事件监听器
    this.$bus.$off("show-use-car-dialog", this.showDialog);
  },
  methods: {
    load() {
    ...
    async submit() {
      ...
      this.$bus.$emit("show-pay-dialog", this.ruleForm)
    },
    handleClose() {
